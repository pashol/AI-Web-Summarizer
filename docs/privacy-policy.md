...
AI Web Summarizer is a Firefox browser extension that uses third-party AI APIs (OpenAI or OpenRouter) to generate summaries and fact-checks of web pages the user is actively viewing. This policy explains exactly what data is collected, why, how it is used, and how it is protected.
...
| Main body **text content** (up to 10,000 characters; scripts, ads, nav removed) | Sent to AI API to generate the summary or fact-check | No — in-memory only during request |
...
- **Access your stored data:** Open the extension Settings panel to view saved API key and preferences.
...
| `<all_urls>` (content script) | Inject the content extraction script into any page so it can extract text when you request a summary; no data is collected passively |
