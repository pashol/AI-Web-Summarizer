...
browser.runtime.onInstalled.addListener(async () => {
  createContextMenu();
  const data = await browser.storage.local.get(['apiKey', 'apiKeys']);
  if (data.apiKey && !data.apiKeys) {
    await browser.storage.local.set({ apiKeys: { openrouter: data.apiKey, openai: '' } });
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'translatePage') {
    handleTranslatePageFromPopup(request.tab).then(result => {
      sendResponse(result);
    }).catch(error => {
      sendResponse({ error: error.message });
    });
  ...
  if (request.action === 'factCheckPage') {
    handleFactCheckPageFromPopup(request.tab).then(result => {
      sendResponse(result);
    }).catch(error => {
      sendResponse({ error: error.message });
    });
...
async function handleTranslateRequest(tab, openInWindow, contextMenuSelection = null) {
  ...
  const newWindow = await browser.windows.create({
    url: browser.runtime.getURL("result.html") + "?mode=translate",
    type: "popup",
    width: 600,
    height: 700
  });
...
async function handleFactCheckRequest(tab, selectedText) {
  ...
  const newWindow = await browser.windows.create({
    url: browser.runtime.getURL("result.html") + "?mode=factcheck",
    type: "popup",
    width: 600,
    height: 700
  });
